...
```
```{r}
distinct_games <- games %>% select(name, developers, publishers) %>% distinct()
```
...
clean_pubs[ rowSums(clean_pubs[3:39]) == 0,]
...
dummy_cols(games, select_columns = platform_name)
```
